...
class emb_model(object):
    def __init__(self, args, d, logdir):
        ...
        self.L = d.L
        self.T = d.T
        self.time_interval = d.time_interval
        ...
        with open(os.path.join(self.logdir,  "log_file.txt"), "a") as text_file:
            text_file.write(str(self.args)+'\n Time Interval used:'+str(self.time_interval))
            text_file.write('\n')
    ...
    def print_word_similarities(self, words, num):
        ...
        val_rho, idx_rho = tf.nn.top_k(tf.matmul(tf.nn.l2_normalize(self.rho, axis=0), tf.nn.l2_normalize(self.alpha, axis=1),
                                                 transpose_b=True)[query_word,:], num)
        ...
        for x in words:
            ...
            with open(f_name, "w+") as text_file:
                ...
                text_file.write("\n\n=====================================\n%s\n=====================================" % (x))
                for ii in range(num):
                    text_file.write("\n%-20s %6.4f" % (self.labels[ir[ii]], vr[ii]))
                with open(os.path.join(self.logdir, "log_file.txt"), "a") as text_filel:
                    text_filel.write('\n')
                    text_filel.write('ir: '+ str(ir))
                    text_filel.write('\n')
                    text_filel.write('vr: '+ str(vr))
    ...
    def train_embeddings(self):
        ...
        self.print_word_similarities(self.query_words, 100)
        if self.dynamic:
...
class dynamic_bern_emb_model(emb_model):
    def __init__(self, args, d, logdir):
        ...
        with tf.name_scope('model'):
            ...
            with tf.name_scope('likelihood'):
                ...
                for t in range(self.T):
                    # Index Masks
                    p_mask = tf.cast(tf.range(self.cs/2,self.n_minibatch[t] + self.cs/2),tf.int32)
                    rows = tf.cast(tf.tile(tf.expand_dims(tf.range(0, self.cs/2),[0]), [self.n_minibatch[t], 1]),tf.int32)
                    columns = tf.cast(tf.tile(tf.expand_dims(tf.range(0, self.n_minibatch[t]), [1]), [1, int(self.cs/2)]),tf.int32)
    ...
    def print_word_similarities(self, words, num):
        ...
        val_rho, idx_rho = tf.nn.top_k(tf.matmul(tf.nn.l2_normalize(query_rho_t, axis=0), tf.nn.l2_normalize(self.alpha, axis=1),
                                                 transpose_b=True)[query_word,:],num)
